...
table.hospitalsView thead th:first-child {
  ...
  <body>
    <nav class="sidenav">
      ...
      <a href="{{route('viewFeedbacks')}}" style="background-color: #76D7C4; border-radius: 10px 0px 0px 10px; border-right: solid 4px #979A9A;"><i class="fa-solid fa-comment-dots"></i> View feedbacks</a>
    ...
    <section class="main">
        ...
        <table class="hospitalsView">
          ...
          @foreach($UserFeedbacks as $UserFeedback)
              <tr><td>{{$UserFeedback->User->name}}</td><td>{{$UserFeedback->reaction}}</td><td>{{$UserFeedback->content}}</td></tr>
          @endforeach
       {{--   @else
          <div>
            <h1 style="color:#243b55; margin:50px 0px 50px 350px; color:#243b55">There's no Users feedbacks right now!</h1>
        </div>
        @endif --}} 
            </tbody>
            </tr>
            ...
            <h1 style="margin-top: 50px;">Hospitals' feedbacks</h1>
            <table class="hospitalsView">
                <thead>
                <tr>
                ...
                <tbody>
             {{--   @if($HospitalFeedbacks->isNotEmpty()) --}} 
              @foreach($HospitalFeedbacks as $HospitalFeedback)
                  <tr><td>{{$HospitalFeedback->Hospital->Hospital_name}}</td><td>{{$HospitalFeedback->reaction}}</td><td>{{$HospitalFeedback->content}}</td></tr>
              @endforeach
           {{--   @else 
    <div>
      <h1 style="color:#243b55; margin:50px 0px 0px 350px; color:#243b55">There's no hospitals feedbacks right now!</h1>
    </div>
@endif --}} 
                </tbody>
                </tr>
